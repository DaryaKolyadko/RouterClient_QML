...
void SocketController::init()
{
    ...
    QObject* accountSettingsInner = accountSettingsSubtab->findChild<QObject*>("accountSettingsInner");
    accountSettingsLoginTextInput = accountSettingsInner->findChild<QObject*>("accountSettingsLoginTextInput");
...
QString SocketController::getInfo(QString message)
{
    if (!connectionLost)
    {
        QString result = socket.writeQueryAndReadAnswer(message);
...
QString SocketController::getParamInfo(QString paramName)
{
    if (!connectionLost)
    {
        QString result = socket.writeQueryAndReadAnswer("get" + paramName);
...
{
    QString result = socket.writeQueryAndReadAnswer(QString("auth %1 %2").arg(login, password));
...
bool SocketController::isConnectionLost()
{
